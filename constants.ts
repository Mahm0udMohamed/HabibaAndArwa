...
const getBasePrompt = (character: Character): string => {
  ...
  switch (character) {
    case Character.Habiba:
      characterInstruction = `اللاعب يلعب بشخصية اسمها "حبيبة"، في الصف الثالث الإعدادي. القاعدة الأهم: ممنوع نهائياً مناداة الشخصية باسم "حبيبة". بدلًا من ذلك، استخدم لقبًا مختلفًا من ألقاب الدلع هذه في كل مرة تناديها: (${nicknamesHabiba}).`;
      break;
    case Character.Arwa:
      characterInstruction = `اللاعب يلعب بشخصية اسمها "أروي"، في الصف الثاني الإعدادي. القاعدة الأهم: ممنوع نهائياً مناداة الشخصية باسم "أروي". بدلًا من ذلك، استخدم لقبًا مختلفًا من ألقاب الدلع هذه في كل مرة تناديها: (${nicknamesArwa}).`;
      break;
    case Character.Both:
      characterInstruction = `اللاعب يلعب بشخصيتي "حبيبة" (في الصف الثالث الإعدادي) و"أروي" (في الصف الثاني الإعدادي) معاً. عند مناداتهما، استخدم نداءً جماعياً مختلفاً كل مرة مثل: (${nicknamesBoth}). ممنوع مناداة كل شخصية باسمها.`;
      break;
  }
