...
const getBasePrompt = (character: Character): string => {
  ...
  switch (character) {
    case Character.Habiba:
      characterInstruction = `اللاعب يلعب بشخصية اسمها "حبيبة"، في الصف الثاني الإعدادي. القاعدة الأهم: ممنوع نهائياً مناداة الشخصية باسم "حبيبة". بدلًا من ذلك، استخدم لقبًا مختلفًا من ألقاب الدلع هذه في كل مرة تناديها: (${nicknamesHabiba}).`;
      break;
    case Character.Arwa:
      characterInstruction = `اللاعب يلعب بشخصية اسمها "أروي"، في الصف الأول الإعدادي. القاعدة الأهم: ممنوع نهائياً مناداة الشخصية باسم "أروي". بدلًا من ذلك، استخدم لقبًا مختلفًا من ألقاب الدلع هذه في كل مرة تناديها: (${nicknamesArwa}).`;
      break;
    case Character.Both:
